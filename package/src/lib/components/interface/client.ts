...
export function createClient<T>(
	...
	for (const [key, value] of Object.entries(input)) {
		if ('method' in value && 'cb' in value) {
			obj[key] = handleClient(value as Route<typeof obj[typeof key][0]['input'],ReturnType<typeof obj[typeof key]['0']>,typeof obj[typeof key][0]['input']['context']>,key,routePrefiex)
		}else{
...
function handleNestedClient<T>(input:SingleOrMultipleRoutes,key:string,routePrefiex:`/${string}`) {	
	const obj:any ={}
	for(const [nestedKey,value] of Object.entries(input)){
		if ('method' in value && 'cb' in value) {
			obj[nestedKey] = handleClient(value as  Route<typeof obj[typeof key]['0']['input'],ReturnType<typeof obj[typeof key]['0']>,typeof obj[typeof key][0]['input']['context']>,`${key}.${nestedKey}`,routePrefiex)
		}else{
